...
log_info()    { (( BOOTSTRAP_VERBOSE )) && _log_emit INFO    "36" stdout "$@" || true; }
log_warn()    { _log_emit WARN    "33" stderr "$@"; }
log_error()   { _log_emit ERROR   "31" stderr "$@"; }
log_success() { _log_emit SUCCESS "32" stdout "$@"; }
log_debug()   { (( BOOTSTRAP_DEBUG )) && _log_emit DEBUG   "90" stdout "$@" || true; }

# Always printed regardless of --verbose. Used for stage transitions and final summary.
log_step()    { _log_emit STEP    "1;34" stdout "$@"; }
log_done()    { _log_emit DONE    "1;32" stdout "$@"; }
