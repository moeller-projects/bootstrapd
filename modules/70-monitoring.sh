...
mod_70_monitoring_description() { echo "Monitoring: btop, fastfetch, smartctl, vnstat, iotop, iftop, lm-sensors"; }
mod_70_monitoring_stage()       { echo "6"; }
mod_70_monitoring_dependencies(){ echo "50-dev"; }
...
_install_gh_binary() {
  local repo="$1" asset_pattern="$2" dest_name="$3" dest_path="$4"
  local tmp
  tmp="$(mktemp -d)"
  ...
  curl --fail --location --silent --show-error --connect-timeout 15 --max-time 600 \
    -o "${tmp}/artifact" -- "$url" || { rm -rf "$tmp"; return 1; }
...
mod_70_monitoring_validate() {
  command -v btop >/dev/null || { log_error "btop missing"; return 1; }
  command -v smartctl >/dev/null || { log_error "smartctl missing"; return 1; }
  command -v vnstat >/dev/null || { log_error "vnstat missing"; return 1; }
  return 0
}
